#!/bin/bash
# Renders every docs/design/diagrams/*.mmd to docs/design/images/<name>.png with
# mermaid-cli, and records the source hash so a stale render is a check failure
# rather than a picture that quietly disagrees with the text around it. Ported
# from the synapse repo's docs/generate-diagrams.sh (same mechanism, two
# directories here instead of one -- diagram sources live under docs/design/
# next to the .excalidraw files they replace, images stay in the existing
# docs/design/images/ alongside the mechanically-generated PNGs).
#
...
if [ ${#SOURCES[@]} -eq 0 ]; then
    echo "generate-diagrams: no .mmd sources in $DIAGRAMS" >&2
    exit 1
fi
...
if [ "$MODE" = "check" ]; then
    if [ ${#STALE[@]} -gt 0 ]; then
        ...
        printf '  %s\n' "${STALE[@]}" >&2
        echo "  run docs/design/generate-diagrams.sh" >&2
        exit 1
    fi
    exit 0
fi
...
for src in "${SOURCES[@]}"; do
    name="$(basename "$src" .mmd)"
    want="$(sha256 < "$src")"
    if [ "$MODE" = "changed" ] && [ -f "$IMAGES/$name.png" ] && [ "$(stamp_of "$name")" = "$want" ]; then
        continue
    fi
    mmdc --quiet -i "$src" -o "$IMAGES/$name.png" -b white -s 2 >/dev/null
